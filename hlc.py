...
class JaccardSimilarityCalculator(object):
    ...
    def __init__(self, graph):
        self._adjlist = [set(graph.neighbors(i, mode="in")).union([i])
...
class TanimotoSimilarityCalculator(object):
    ...
    def __init__(self, graph, attr="weight"):
        ...
        for i in range(graph.vcount()):
            weis = dict((j, weights[get_eid(i, j)]) for j in graph.neighbors(i, mode="in"))
            # zz = degrees[i]
            # if zz==0:
            #     print("zero division avoided.")
            #     zz = 1
            weis[i] = strengths[i] / degrees[i]
            # weis[i] = strengths[i] / zz
...
class HLC(object):
    ...
    def get_edge_similarity_graph(self):
        ...
        # print("Construct the line graph...")
        ...
        if "weight" in self.graph.edge_attributes():
            ...
            # print("Processing with Tanimoto similarity index")
        else:
            # print("Processing with Jaccard similarity index")
        ...
        # print("Computing similary score...")
        scores = []
        ...
        for edge in linegraph.es:
            ...
            if a == c:
                ...
                # print(s,edge.source, edge.target)
    ...
    def _run_iterative(self):
        ...
        # print("self._edgelist=",self._edgelist)
        ...
        for edge in sorted_edges:
            ...
            # print(self._edgelist[edge.source][0]+1,self._edgelist[edge.source][1]+1,"-",self._edgelist[edge.target][0]+1,self._edgelist[edge.target][1]+1," = ", edge["score"])
            # print(edge.source, " - ", edge.target, " = ", edge["score"])
            ...
            if prev_score != score:
                ...
                # print("... density = ", clusters.d * 2 / self.graph.ecount())
...
def hlc_single(graph, threshold=0.66, min_size=3):
    ...
    adjlist = [set(graph.neighbors(i, mode="in")).union([i])
               for i in range(graph.vcount())]
    ...
    clusters = linegraph.clusters()
    result = [set() for _ in range(len(clusters))]
    for edge, cluster_index in zip(edgelist, clusters.membership):
        result[cluster_index].update(edge)
